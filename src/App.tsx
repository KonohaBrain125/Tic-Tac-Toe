...
import { winningCombinations } from '@const/index';

export type SquaresValueProps = 'X' | 'O' | null;

export const App = () => {
  const [isXNext, setIsXNext] = useState<boolean>(true);
  const [squares, setSquares] = useState<SquaresValueProps[]>(Array(9).fill(null));
  const [winningSquares, setWinningSquares] = useState<number[]>(Array(3).fill(null));
  ...
  const handleClick = (i: number) => {
    if (squares[i] || winningSquares.every((square) => square !== null)) {
      setEnd(true);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (winner === 'X') {
      setXWins((prevXWin) => prevXWin + 1);
    } else if (winner === 'O') {
      setOWins((prevOWin) => prevOWin + 1);
    } else if (winner === 'Draw') {
      setDraws((prevDraw) => prevDraw + 1);
    }
  }, [squares]);
  ...
  const handleRestart = () => {
    ...
    setWinningSquares(Array(3).fill(null));
    setEnd(false);
  };

  return (
    <div className="flex flex-col gap-5 p-2 h-screen justify-center items-center">
      ...
      <div className="flex flex-row items-center gap-x-20">
        {winningSquares.every((winningSquare) => winningSquare !== null) ? (
          ...
          <div
            className={`transition-all p-3 rounded-full w-12 h-12 flex items-center justify-center ${isXNext ? 'bg-sky-600' : 'bg-cyan-500 text-sky-50 scale-125'}`}
          >
            {isXNext ? (
              <svg
                xmlns="http://www.w3.org/2000/svg"
